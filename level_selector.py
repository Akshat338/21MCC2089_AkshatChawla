...
def draw(screen,level_completed,record):
	...
	for i in range(level_completed+1):
		...
		if button_col==3:
			...
			button_row+=1
			'''
	for i in range(level_completed):
		tile_button = button.Button((100 * abutton_col) + 100 , (100 * abutton_row + 180), active_img_list[i], 1)
		abutton_list.append(tile_button)
		abutton_col+=1
		if abutton_col==3:
			abutton_col=0
			abutton_row+=1
			'''
	play_button=button.Button(50 , 450, play_img, 1)	
	help_button=button.Button(50 , 300, help_img, 1)	
	highscore_button=button.Button(50 , 150, highscore_img, 1)
	highscore_button=button.Button(50 , 150, highscore_img, 1)		
	while run:	
		...
		for rec in record:
			for x in range(8,level_completed+9):
				...
				label.draw(f'{rec[x+9]}',(255,255,255),(100 * button_col) + 460 , (150 * button_row) + 200,screen,24,'Copper')	
				button_col+=1
				if button_col==3:
					button_col=0
					button_row+=1
		...
		for event in pygame.event.get():
			if event.type==pygame.QUIT:
				return False,False,current_level
